...
if __name__ == "__main__":
    ...
    VE = Evaluator("hyphen_dict.json", original_comedy_path="data/divina_textonly.txt")
    ...
    global_log_path = f"{root_dir}{root_dir.split('/')[-2]}__results.txt"
    results_table_path = "table_*.csv".replace("*", root_dir.split("/")[0])
    ...
    for epochs in [name for name in os.listdir(root_dir) if os.path.isdir(root_dir+name)]: # epoch folder: 0_10, 0_20, ...
        ...
        for mode in [name for name in os.listdir(path) if os.path.isdir(path+name)]: # mode: "sampling"/"beam search"
            ...
            for n,filename in enumerate(generation_files):
                ...
                # verses lenght
                verses_lenghts = {key:round(verses_lenghts[key]/n_verses, 2) for key in sorted(verses_lenghts.keys())}
                ...
                # plagiarism score
                plag_score = VE.plagiarism_score(text) if hendec_score >= 0.95 else -1 # evaluate only good candidates
